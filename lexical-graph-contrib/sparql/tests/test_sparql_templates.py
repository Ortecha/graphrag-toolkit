...
def test_topic_content_returns_statement_details():
    ...
    assert '?fact lg:supports ?statementNode .' in client.queries[0]
    assert 'OPTIONAL { ?fact lg:supports ?statementNode }' not in client.queries[0]
...
def test_entities_for_chunk_ids_scores_entities_in_matching_chunks():
    ...
    assert 'VALUES ?chunkId { "chunk-1" }' in client.queries[0]
    assert 'lg:statementMentionedIn ?chunk' in client.queries[0]
    assert 'SELECT DISTINCT ?entity WHERE' in client.queries[0]
    assert '?matchedFact lg:supports ?statement' in client.queries[0]
    assert '?fact ?factPredicate ?entity' in client.queries[0]
